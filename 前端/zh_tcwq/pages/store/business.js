...
Page({
    ...
    selectDistrictChildren: function(t) {
        var e = t.currentTarget.dataset.index,
            i = -1 == this.data.activeDistrictParentIndex ? 0 : this.data.activeDistrictParentIndex;
        0 == e ? this.setData({
    ...
    selectType: function(t) {
        var e = this;
        if (t.currentTarget.dataset.index==0){
          var i = 0;
        }else{
              ...
              console.log(t)
                for (var i in e.setData({
                    ...
                    var n = Number(wx.getStorageSync("Location").latitude),
                        r = Number(wx.getStorageSync("Location").longitude),
                        s = a[0],
                        g = a[1],
                        d = n * Math.PI / 180,
                        h = s * Math.PI / 180,
                        l = d - h,
                        u = r * Math.PI / 180 - g * Math.PI / 180,
                        p = 2 * Math.asin(Math.sqrt(Math.pow(Math.sin(l / 2), 2) + Math.cos(d) * Math.cos(h) * Math.pow(Math.sin(u / 2), 2)));
                    p *= 6378.137;
