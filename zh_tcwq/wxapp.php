...
echo json_encode($data2);
    ...
    //总浏览量
     public function doPageViews(){
      global $_W, $_GPC;
      $sys=pdo_get('zhtc_system',array('uniacid'=>$_W['uniacid']),'zfwl_max');
      $rand=empty($sys['zfwl_max'])?1:$sys['zfwl_max'];
      pdo_update('zhtc_system',array('total_num +='=>rand(1,$rand)),array('uniacid'=>$_W['uniacid']));
      $system=pdo_get('zhtc_system',array('uniacid'=>$_W['uniacid']));
      echo ($system['total_num']);
    }
   //帖子总量
   ...
   //商家列表
    ...
    //分类下商家列表
          ...
          //   }
          // }
          /*$res=pdo_getall('zhtc_store',array('uniacid'=>$_W['uniacid'],'time_over !='=>1,'storetype_id'=>$_GPC['storetype_id'],'state'=>2),array(),'','id desc');*/
...
public function doPageVideoList(){
    ...
    $sql="select a.* from" . tablename("zhtc_video") . " a"  . " left join " . tablename("zhtc_videotype") . " b on b.id=a.type_id ".$where."  ORDER BY a.num asc";
    $select_sql =$sql." LIMIT " .($pageindex - 1) * $pagesize.",".$pagesize;
    $res = pdo_fetchall($select_sql,$data);
...
public function doPageJfGoods(){
  global $_W, $_GPC;
  $res=pdo_getall('zhtc_jfgoods',array('uniacid'=>$_W['uniacid']),array(),'','num asc');
  echo json_encode($res);
}
 //商品详情
...
public function doPageJftypeGoods(){
  global $_W, $_GPC;
  $res=pdo_getall('zhtc_jfgoods',array('type_id'=>$_GPC['type_id']),array(),'','num asc');
  echo json_encode($res);
}
//积分商城广告
