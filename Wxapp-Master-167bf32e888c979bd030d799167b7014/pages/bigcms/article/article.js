...
Page({
  ...
  getArticleInfo: function (isShowLoading) {
    ...
    requestUtil.get(_DuoguanData.duoguan_host_api_url + "/index.php?s=/addon/DuoguanBigCms/Api/getArticleInfo.html", requestData, (data) => {
      ...
      //开启音频自动播放
      if (that.data.is_avAutoPlay == 1 && that.data.typeid == 2 && that.data.info_data.pddata.is_userpay==1) {
  ...
  formSubmit: function (e) {
    ...
    //查看是否同步用户信息
    requestUtil.get(_DuoguanData.duoguan_host_api_url + "/index.php?s=/addon/DuoguanBigCms/Api/getUserTBstatus.html", {}, (data) => {
      if (data==0) {
        //未同步头像昵称
        this.onSyncWechatInfo(formData);
      } else {
        that.toComment(formData);//去评论
      }
    }, this, {});
  ...
  //返回首页
  toIndex: function (e) {
    let url = '/pages/bigcms/index/index';
    wx.switchTab({
      url: url,
      fail: (res) => {
        if (is_super == 1) {
  ...
  /**
     * 同步微信信息
     */
  onSyncWechatInfo: function (formData) {
    var that = this;
